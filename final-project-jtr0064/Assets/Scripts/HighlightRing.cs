...
// Glowing shell dropped over a target's own hitbox while a highlight ability is active - see
// PlayerAbilities.OnAbilityTwo (resources, gold) and PlayerAbilities.OnAbilityFour (dropoff
// locations, light blue). The shell is a primitive (sphere/capsule/box) sized and positioned
// to match the target's Collider exactly, so the thing that lights up is the actual
// interaction hitbox rather than an arbitrary ring under the model. Uses Custom/HighlightXRay
// (Assets/Shaders/HighlightXRay.shader) so the glow is visible through walls and through the
// target's own mesh - that's the point of the ability.
public class HighlightRing : MonoBehaviour
{
    public static readonly Color GoldColor = new Color(1f, 0.82f, 0.15f);
    public static readonly Color LightBlueColor = new Color(0.5f, 0.8f, 1f);
    ...
    public static HighlightRing Attach(Transform target, Color color)
    {
        ...
        HighlightRing ring = go.AddComponent<HighlightRing>();
        ring.Init(color);
        return ring;
    }
